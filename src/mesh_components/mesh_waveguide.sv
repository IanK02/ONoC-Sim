...
module mesh_waveguide #(
    parameter DELAY = 4, 
    parameter MOD_DELAY = 2,
    ...
    integer i;
    assign packet_done[0] = valid_right[MOD_DELAY-2];
    assign packet_done[1] = valid_left[MOD_DELAY-2];
    ...
    always_ff @(posedge clk) begin
        ...
        $write("  L->R  ");
        for (i = 0; i < DELAY; i++) begin
            $write("[%0d:%s:%0d-%0d] ", i, valid_right[i] ? "V" : "", buffer_right[i].data[31:24],buffer_right[i].data[23:0]);
        end
        $write("\n");
        $write("  R->L  ");
        for (i = 0; i < DELAY; i++) begin
            $write("[%0d:%s:%0d-%0d] ", i, valid_left[i] ? "V" : "", buffer_left[i].data[31:24],buffer_left[i].data[23:0]);
        end
        $write(" Connecting router %0d,%0d with router %0d,%0d", router_a,dir_a,router_b,dir_b);
        $write("\n");
    end
